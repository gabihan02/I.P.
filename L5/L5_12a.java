// (a) A filial que mais vendeu durante o semestre 
...
class L5_12a{
	...
	public static void main(String[] args){
		...
		System.out.println(MesMaisVendeu(vendas));
	...
	static double ValoresLinha(double[][] num, int linha){
		...
		int fim = 0;
		for (int i=0; i<num.length; i++){ // linha que é fixo
			if (fim < num[linha][i]) fim += num[linha][i];
		}
		return (fim);
	}
	
	
	static double MesMaisVendeu(double[][] num){		
		int indiceMax = 0;
		double somaMaior = ValoresLinha(num, indiceMax);
